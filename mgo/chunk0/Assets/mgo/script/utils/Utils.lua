...
e.TutorialPhase=0
e.TutorialTime=0
e.weatherStartIds={}e.weatherEndIds={}e.WEATHER_CHANGE_COUNT_MAX=8
...
e.GEARPOINTS={WINNER=300,LOSER=100,TIE=200}function e.IsBaseRuleset(e)return 100<=e
end
function e.IsRuleset(p,e)return MpRulesetManager.GetRulesetScriptVarsValueById(p)==e
end
function e.DisplayError(p)e.LastLogYLocation=e.LastLogYLocation+e.LogLineHeight
if e.LastLogYLocation>e.LogLineMaxY then
e.LastLogYLocation=e.LogLineMinY
end
end
function e.DisplayLog(p,_,i,n,T,o)e.DisplayLogColor(p,_,i,n,T,o,Color{0,0,0,1})end
function e.DisplayLogColor(o,T,p,n,_,i,o)local o=60
if p~=nil then
o=p
end
local p=6
...
function e.StringArrayToCSV(p)local e=""local _=false
for i=1,#p do
local p=p[i]if p~=nil then
if _==false then
e=e..p
_=true
else
...
function e.GetTeamTag(p)local e="TEAM_"if p<10 then
e=e.."0"end
e=e..tostring(p+1)return e
end
function e.GetInstanceTag(p)local e="GENERIC_"if p<10 then
e=e.."0"end
e=e..tostring(p)return e
end
function e.StringStartsWith(p,e)return string.sub(p,1,string.len(e))==e
end
function e.ExtractInstanceTag(p)for _=1,#p do
if e.StringStartsWith(p[_],"GENERIC_")then
return p[_]end
end
return nil
end
function e.RemoveStringFromStringArray(e,i)for _,p in ipairs(e)do
if p==i then
table.remove(e,_)return
end
end
end
function e.FindInArray(e,p)for _,e in ipairs(e)do
if e==p then
return _
end
end
...
function e.InsertArrayUniqueInArray(_,p)for i=1,#_ do
local _=_[i]if e.FindInArray(p,_)==0 then
p[#p+1]=_
end
end
end
function e.SetWeatherInterval(o,T,i,a,_,n)for p=1,e.WEATHER_CHANGE_COUNT_MAX do
if e.weatherStartIds[p]then
Util.ClearInterval(e.weatherStartIds[p])end
if e.weatherEndIds[p]then
Util.ClearInterval(e.weatherEndIds[p])end
end
for p=1,#i do
e.weatherStartIds[p]=Util.SetInterval(i[p]*1e3,false,"Utils","SyncWeather",T,_/n)e.weatherEndIds[p]=Util.SetInterval(((i[p]+a)+_)*1e3,false,"Utils","SyncWeather",o,_/n)end
end
function e.SyncWeather(e,p)local p=p*1e3
local _=MpRulesetManager.GetActiveRuleset()if e=="clear"then
_:RequestWeather(0,p)elseif e=="cloudy"then
_:RequestWeather(1,p)elseif e=="rainy"then
_:RequestWeather(2,p)elseif e=="sandstorm"then
_:RequestWeather(3,p)elseif e=="foggy"then
_:RequestWeather(4,p)end
end
function e.TriggerEffect(_,i,e,p)for e=i,e do
if e<10 then
TppDataUtility.SetVisibleEffectFromId(_..("0"..tostring(e)),p)else
TppDataUtility.SetVisibleEffectFromId(_..tostring(e),p)end
end
end
function e.SetupEffects(_,p)if vars.isGameplayHost==1 then
e.WeatherRequest(_,p,false)end
if _==103 then
if p then
end
elseif _==101 then
...
if p then
TppDataUtility.SetVisibleEffectFromGroupId("FxLocatorGroup_NightTimeOnly",true)TppDataUtility.SetVisibleEffectFromGroupId("FxLocatorGroup_DaytimeOnly",false)else
TppDataUtility.SetVisibleEffectFromGroupId("FxLocatorGroup_NightTimeOnly",false)TppDataUtility.SetVisibleEffectFromGroupId("FxLocatorGroup_DaytimeOnly",true)end
return
end
function e.WeatherRequest(i,n,o)local p={}if math.random(2)==1 then
p={180}end
if MgoMatchMakingManager.GetWeatherChangeState~=nil then
local _=MgoMatchMakingManager.GetWeatherChangeState()if _==0 then
p={}elseif _==2 then
local _=math.random(20,80)p={}while#p<e.WEATHER_CHANGE_COUNT_MAX and _<(vars.roundTimeLimit-30)do
table.insert(p,_)_=(_+math.random(120,240))+(vars.roundTimeLimit/30)end
end
end
local _=false
if o and#p>0 then
...
if i==103 then
if n then
e.SyncWeather("rainy",0)WeatherManager.SetCurrentClock("1","00")else
e.SyncWeather("clear",0)WeatherManager.SetCurrentClock("13","00")if _ then
e.SetWeatherInterval("clear","rainy",p,30,20,2)end
end
elseif i==101 then
if n then
e.SyncWeather("clear",0)WeatherManager.SetCurrentClock("1","00")if _ then
e.SetWeatherInterval("clear","cloudy",p,30,20,1)end
else
e.SyncWeather("clear",0)WeatherManager.SetCurrentClock("15","00")if _ then
e.SetWeatherInterval("clear","cloudy",p,30,20,1)end
end
elseif i==102 then
if n then
e.SyncWeather("clear",0)WeatherManager.SetCurrentClock("1","00")if _ then
e.SetWeatherInterval("clear","sandstorm",p,30,20,2)end
else
e.SyncWeather("clear",0)WeatherManager.SetCurrentClock("06","12")if _ then
e.SetWeatherInterval("clear","sandstorm",p,30,20,2)end
end
elseif i==104 then
if n then
...
e.SyncWeather("cloudy",0)WeatherManager.SetCurrentClock("06","30")if _ then
e.SetWeatherInterval("cloudy","rainy",p,30,20,1)end
end
elseif i==105 then
if n then
e.SyncWeather("cloudy",0)WeatherManager.SetCurrentClock("1","00")if _ then
e.SetWeatherInterval("cloudy","rainy",p,30,20,1)end
else
e.SyncWeather("cloudy",0)WeatherManager.SetCurrentClock("17","30")if _ then
e.SetWeatherInterval("cloudy","rainy",p,30,20,1)end
end
...
function e.AssignWeaponsLoadout(p,T,a,n,o)local _=(p)*3
local i=(p)*18
...
function e.OneMinuteLeft()TppUiCommand.AnnounceLogViewLangId"mgo_announce_generic_one_min"end
function e.TryNotifyBuddyChange(p,e,_,i)if SpawnHelpers.initialTeamAssignmentHasHappened==true and p.currentState~="RULESET_STATE_BRIEFING"then
TppNetworkUtil.SyncedExecuteSessionIndex(_,"Utils","announceBuddyLink",e)TppNetworkUtil.SyncedExecuteSessionIndex(i,"Utils","announceBuddyLink",e)end
end
function e.announceBuddyLink(e)if e then
...
function e.DetermineMatchWinner(T,i,o,n,_)local p=0
SpawnHelpers.Reset()if o==i then
p=i
elseif n>_ then
p=e.Team.SOLID
elseif n<_ then
...
local e=e.BreakTie(T,false)if e==0 then
p=1
else
p=0
end
...
function e.BreakTie(n,a)local T
local o=-1
local _
local i=-1
local p
...
for p=1,#e do
local e=e[p]if not n:IsDedicatedHost(e.sessionIndex)then
local p
if a==true then
p=n:GetStatByPlayerIndex(e.sessionIndex,MgoStat.STAT_TEAM_POINTS)else
p=n:GetTotalStatByPlayerIndex(e.sessionIndex,MgoStat.STAT_TEAM_POINTS)end
if p>o then
o=p
T=e.sessionIndex
_=e.teamIndex
...
function e.AwardGearPointsToAllPlayers(p,n)for _,i in pairs(SpawnHelpers.teamRoster)do
if not p:IsDedicatedHost(_)then
if i==n then
p:IncrementStatByPlayerIndex(_,MgoStat.STAT_GEAR_POINTS,e.GEARPOINTS.WINNER)else
p:IncrementStatByPlayerIndex(_,MgoStat.STAT_GEAR_POINTS,e.GEARPOINTS.LOSER)end
end
end
end
function e.InitTutorial()e.TutorialPhase=0
e.TutorialTime=0
end
function e.DrawTutorial(p,_)local i=p.currentState
...
local p=p:GetTimeSpentInCurrentState()if p>5 then
return
end
e.TutorialPhase=1
e.TutorialTime=p
elseif e.TutorialPhase==1 then
local p=p:GetTimeSpentInCurrentState()if TppUiCommand.IsMbDvcTerminalOpened()or TppUiCommand.IsOpenCustomizeMenu()then
e.TutorialTime=p-5
else
...
function e.SetStaminaConfig(p)local e={Stamina={UnconsciousStaminaRegenRate=.35,BasicActionStaminaRegenRate=1,ComplexActionStaminaRegenRate=1,ButtonMashModifier=.75}}p:ReloadRulesetConfig(e)end
return e