...
function e.StartTitle(e)vars.rulesetId=4
vars.locationCode=101
vars.missionCode=6
TppMission.ResetNeedWaitMissionInitialize()gvars.ini_isTitleMode=true
gvars.ini_isReturnToTitle=false
TppPlayer.SetStartStatus(TppDefine.INITIAL_PLAYER_STATE.ON_FOOT)vars.initialPlayerAction=PlayerInitialAction.STAND
TppPlayer.ResetDisableAction()TppPlayer.ResetInitialPosition()TppPlayer.ResetMissionStartPosition()TppMission.ResetIsStartFromHelispace()TppMission.ResetIsStartFromFreePlay()TppMission.VarResetOnNewMission()if clock then
TppClock.SetTime(clock)TppClock.SaveMissionStartClock()end
TppSimpleGameSequenceSystem.Start()TppMission.Load(vars.missionCode,currentMissionCode)local e=Fox.GetActMode()if(e=="EDIT")then
...
local e={}local t=1*i
local a=TppGameSequence.GetTargetPlatform()if a=="Steam"then
t=2*i
end
local a=t
...
Tpp.DEBUG_DumpTable(e)TppScriptVars.CreateSaveSlot(e)TppScriptVars.SetFileSizeList{{TppDefine.CONFIG_SAVE_FILE_NAME,a},{TppDefine.PERSONAL_DATA_SAVE_FILE_NAME,t},{TppDefine.MGO_SAVE_FILE_NAME,i}}end
...
TppMotherBaseManagement.DirectAddResource{resource="Plant2005",count=20,isNew=true}gvars.solface_groupNumber=(math.random(0,255)*65536)+math.random(1,255)gvars.hosface_groupNumber=(math.random(0,65535)*65536)+math.random(1,65535)local t,i
t={{slot=TppDefine.WEAPONSLOT.PRIMARY_HIP,equip=TppEquip.EQP_None},{slot=TppDefine.WEAPONSLOT.SECONDARY,equip=TppEquip.EQP_None},{slot=TppDefine.WEAPONSLOT.SUPPORT_0,equip=TppEquip.EQP_None},{slot=TppDefine.WEAPONSLOT.SUPPORT_1,equip=TppEquip.EQP_None}}i={TppEquip.EQP_None,TppEquip.EQP_None,TppEquip.EQP_None,TppEquip.EQP_None}e.SetInitPlayerWeapons(t)TppPlayer.SupplyAllAmmoFullOnMissionFinalize()e.SetInitPlayerItems(i)e.InitializeAllPlatformForNewGame()end
...
TppMission.VarResetOnNewMission()TppPlayer.ResetInitialPosition()TppPlayer.SetStartStatus(TppDefine.INITIAL_PLAYER_STATE.ON_FOOT)TppSave.VarSave(nil,true)TppSave.VarSaveConfig()TppSave.VarSavePersonalData()TppMission.Load(vars.locationCode,nil,{force=true,showLoadingTips=false})local e=Fox.GetActMode()if(e=="EDIT")then
Fox.SetActMode"GAME"end
end
...
local a=1
local i={"Command","Combat","Develop","BaseDev","Support","Spy","Medical"}local e={"MotherBase","Fob1","Fob2","Fob3","Fob4"}for a,e in ipairs(e)do
for a,i in ipairs(i)do
TppMotherBaseManagement.SetClusterSvars{base=e,category=i,grade=t,buildStatus="Completed",timeMinute=0,isNew=false}end
end
