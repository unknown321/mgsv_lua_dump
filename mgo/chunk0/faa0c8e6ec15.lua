...
TppSave.SaveConfigData()end
TppSave.VarSaveMGO()if TppSave.IsSavingWithFileName(TppDefine.MGO_SAVE_FILE_NAME)or TppSave.HasQueue(TppDefine.MGO_SAVE_FILE_NAME)then
else
TppSave.SaveMGOData()end
end,option={isExecGameOver=true}},{msg="MgoConfigurationUpdated",func=function()TppSave.VarSaveMGO()if TppSave.IsSavingWithFileName(TppDefine.MGO_SAVE_FILE_NAME)or TppSave.HasQueue(TppDefine.MGO_SAVE_FILE_NAME)then
else
TppSave.SaveMGOData()end
end,option={isExecGameOver=true}},{msg="DisplayTimerLap",func=function(e,n)if not mvars.ui_displayTimeSecSvarsName then
return
end
...
local function a(e)if type(e)=="string"then
return Fox.StrCode32(e)elseif type(e)=="number"then
return e
end
return nil
end
function e.FadeIn(t,i,s,n)local i=a(i)if n then
...
TppSoundDaemon.ResetMute"Outro"o(t,i,s)e.EnableGameStatusOnFadeInStart()end
...
function e.FadeOut(s,r,u,n)local i,o,t
if Tpp.IsTypeTable(n)then
i=n.setMute
o=n.exceptGameStatus
t=n.forceNotMute
end
local n=a(r)e.DisableGameStatusOnFade(o)if not t then
if i then
...
_(s,n,u)end
function e.ShowAnnounceLog(a,t,o,i,n)local e=e.ANNOUNCE_LOG_TYPE[a]if e then
if i then
TppUiCommand.AnnounceLogDelayTime(i)end
TppUiCommand.AnnounceLogViewLangId(e,t,o)elseif n then
local e=TppUiCommand.GetCurrentMissionSubGoalByNo(n)TppUiCommand.AnnounceLogViewLangId(e)end
end
...
function e.ShowColorJoinAnnounceLog(n,o,t,a,i)if gvars.ini_isTitleMode then
return
end
local n=e.ANNOUNCE_LOG_TYPE[n]local e=e.ANNOUNCE_LOG_TYPE[o]if n and e then
if i then
TppUiCommand.AnnounceLogDelayTime(i)end
TppUiCommand.AnnounceLogViewJoinLangId(n,e,t,a,0,0,true)end
end
...
function e.EnableMissionPhoto(n,i,e,t,a)TppUiCommand.EnableMissionPhotoId(n)if TppUiCommand.IsMissionPhotoIdEnable(n)then
if i or e then
if i==nil then
i=false
end
...
TppUiCommand.SetAdditonalMissionPhotoId(n,i,e)end
if t then
end
if a then
local e=p(a)TppUiCommand.SetMissionPhotoRadioGroupName(n,e)end
end
end
...
i=e.gameObjectName
n=t(i)elseif Tpp.IsTypeNumber(e.gameObjectId)then
n=e.gameObjectId
else
return
end
...
function e.EnableMissionTask(t,a)if a==nil then
a=true
end
if not i(t)then
return
end
local n={}for i,e in pairs(t)do
n[i]=e
end
local i=n.taskNo
if not i then
...
local o=e.GetTaskCompletedNumber(n)e.SetTaskLastCompleted(i,true)local t=e.GetTaskCompletedNumber(n)local i=e.GetMaxMissionTask(n)if i==nil then
return
end
if a then
if t>o then
e.ShowAnnounceLog("task_complete",t,i)end
end
...
local e=TppDefine.CONTROL_GUIDE[a]local n=TppDefine.CONTROL_GUIDE_LANG_ID_LIST[e]if n==nil then
return
end
if t then
...
TppUiCommand.SetButtonGuideDispTime(i)if not s then
if(o==true)then
...
function e.ShowCallSupportBuddyAnnounceLog()local i=e.BUDDY_LANG_ID[mvars.ui_callSupportBuddyType]if i then
local n=TppSupportRequest.GetCallBuddyGmpCost(mvars.ui_callSupportBuddyType)e.ShowAnnounceLog("callSupportBuddyReceived",i)TppTerminal.UpdateGMP{gmp=-n,gmpCostType=TppDefine.GMP_COST_TYPE.BUDDY}svars.supportGmpCost=svars.supportGmpCost+n
...
TppUiCommand.ResetCurrentMissionSubGoalNo()e._RegisterDefaultLandPoint()if TppUiCommand.IsTppUiReady()then
if i then
e.RegisterHeliSpacePauseMenuPage(true)elseif a then
...
TppUiCommand.RegisterPauseMenuPage{GamePauseMenu.ABORT_MISSION_RETURN_TO_ACC,GamePauseMenu.SIGN_IN,GamePauseMenu.STORE_ITEM,GamePauseMenu.RECORDS_ITEM,GamePauseMenu.CONTROLS_AND_TIPS_ITEM,GamePauseMenu.OPEN_OPTION_MENU}else
TppUiCommand.RegisterPauseMenuPage{GamePauseMenu.ABORT_MISSION_RETURN_TO_ACC}end
else
...
if vars.fobSneakMode==FobMode.MODE_VISIT then
TppUiCommand.RegisterGameOverMenuItems{GameOverMenu.GAME_OVER_RESTART_HELI}else
TppUiCommand.RegisterGameOverMenuItems{GameOverMenu.GAME_OVER_ABORT}end
else
...
TppUiCommand.SetMBMapArrowIcon(false)GameConfig.ApplyAllConfig()TppUiCommand.EraseDisplayTimer()TppUiCommand.ResetCpNameBaseLangId()TppUiCommand.ClearAllMissionTask()TppUiStatusManager.UnsetStatus("EquipHudAll","ALL_KILL_NOUSE")TppUiStatusManager.UnsetStatus("QuestAreaAnnounce","INVALID")TppUiStatusManager.UnsetStatus("SideFobInfo","INVALID")TppUiCommand.SetupMapIconRadiusMeters{radiusTable={0,30,40,62.5,125,200,325,450,600,1200}}TppUiCommand.HidePictureInfoHud()if TppUiCommand.IsTppUiReady()then
local n=TppGameSequence.GetTargetPlatform()local e={}if vars.rulesetId==4 then
table.insert(e,GamePauseMenu.ONLINE_NEWS)table.insert(e,GamePauseMenu.STORE_ITEM)if(n~="XboxOne")then
table.insert(e,GamePauseMenu.OPEN_WEB_MANUAL)end
table.insert(e,GamePauseMenu.CREDIT)table.insert(e,GamePauseMenu.STAFF_CREDIT)table.insert(e,GamePauseMenu.INQUIRY_ID)table.insert(e,GamePauseMenu.OPEN_OPTION_MENU)table.insert(e,GamePauseMenu.GOTO_TPP)else
table.insert(e,GamePauseMenu.INVITE_MENU)if(n=="Xbox360")then
table.insert(e,GamePauseMenu.INVITE_PARTY_MENU)end
table.insert(e,GamePauseMenu.OPEN_OPTION_MENU)table.insert(e,GamePauseMenu.EXIT_MATCH)end
TppUiCommand.RegisterPauseMenuPage(e)end
end
function e.RegisterHeliSpacePauseMenuPage(n)local e={GamePauseMenu.RETURN_TO_TITLE,GamePauseMenu.SIGN_IN,GamePauseMenu.RECORDS_ITEM,GamePauseMenu.CONTROLS_AND_TIPS_ITEM,GamePauseMenu.OPEN_OPTION_MENU,GamePauseMenu.GOTO_MGO}if n then
table.insert(e,3,GamePauseMenu.STORE_ITEM)end
TppUiCommand.RegisterPauseMenuPage(e)end
function e.OnReload()e.Init()end
function e.OnMessage(r,s,t,o,a,i,n)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,r,s,t,o,a,i,n)end
