local e={}local a={}local i={}local c=256
local u=0
local _=180
...
n[e]={name=a,type=TppScriptVars.TYPE_BOOL,value=s,save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}elseif t=="number"then
n[e]={name=a,type=TppScriptVars.TYPE_INT32,value=s,save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}elseif t=="string"then
n[e]={name=a,type=TppScriptVars.TYPE_UINT32,value=o(s),save=true,sync=true,category=TppScriptVars.CATEGORY_MISSION}elseif t=="table"then
n[e]=s
end
e=e+1
end
...
local s=6
local t=2
i={"Seq_Mission_Prepare"}e.SYS_SEQUENCE_LENGTH=#i
...
mvars.seq_canMissionStartWaitStartTime=Time.GetRawElapsedTimeSinceStartUp()TppMain.OnEnterMissionPrepare()TppMain.DisablePause()end,OnLeave=function(s,n)TppMain.OnMissionGameStart(n)e.DoOnEndMissionPrepareFunction()if e.IsFirstLandStart()then
if not TppSave.IsReserveVarRestoreForContinue()then
local e=TppMission.GetMissionStartRecoverDemoType()if e==TppDefine.MISSION_START_RECOVER_DEMO_TYPE.WALKER_GEAR then
TppUI.ShowAnnounceLog("looting_weapon",nil,nil)TppUI.ShowAnnounceLog("get_wgear","walkergear_w_1",1)end
TppMission.UpdateCheckPointAtCurrentPosition()end
end
...
end,DEBUG_TextPrint=function(e)local n=(nil).NewContext();(nil).Print(n,{.5,.5,1},e)end,OnUpdate=function(t)if(mvars.seq_missionPrepareState<e.MISSION_PREPARE_STATE.END_TEXTURE_LOADING)then
...
TppMpBaseRuleset.SetEnabledToSpawnPlayers(false)else
TppMpBaseRuleset.SetEnabledToSpawnPlayers(true)end
end
local c=30
local p=.35
local s=false
local a=false
local i=Mission.GetTextureLoadedRate()local r=TppMission.CanStart()local l=TppMotherBaseManagement.IsEndedSyncControl()if t.SkipTextureLoadingWait()then
i=1
end
local a=0
local t=c
local u=Time.GetRawElapsedTimeSinceStartUp()local S=u-mvars.seq_canMissionStartWaitStartTime
...
TppTerminal.VarSaveMbMissionStartSyncEnd()TppSave.DoReservedSaveOnMissionStart()if r then
...
TppMain.OnTextureLoadingWaitStart()mvars.seq_textureLoadWaitStartTime=u
end
...
function e.OnMessage(o,n,i,r,a,t,s)if mvars.seq_sequenceTable==nil then
return
end
local e=mvars.seq_sequenceTable[mvars.seq_currentSequence]if e==nil then
return
...
Tpp.DoMessage(e,TppMission.CheckMessageOption,o,n,i,r,a,t,s)end
...
if e.debug.showCurrentSequence then(nil).Print(n,{.5,.5,1},"LuaSystem SEQ.showCurrSequence");(nil).Print(n," current_sequence = "..tostring(r(s.seq_sequence)))end
