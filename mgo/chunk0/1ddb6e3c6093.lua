do
return{}end
local n={}function n.SetPlayLogEnabled(e)if TppPlayLog then
TppPlayLog.SetPlayLogEnabled(e)end
end
function n.RequestResetPlayLog()if TppPlayLog then
TppPlayLog.RequestResetPlayLog()end
end
function n.RequestUploadPlayLog()if TppPlayLog then
TppPlayLog.RequestUploadPlayLog()end
end
function n.ExportSavedPlayLog()if TppPlayLog and TppPlayLog.ExportSavedPlayLog then
TppPlayLog.ExportSavedPlayLog()end
end
n.PERF_CHECK_TYPE=Tpp.Enum{"OnUpdate","OnMessage","OnEnter"}local i={}local g={}local l={}local p=2
local t=0
local r=0
...
function n.DEBUG_SetGVars(e)if not s(e)then
return
end
...
function n.DEBUG_RestoreSVars()if next(Tpp._DEBUG_svars)then
for e,a in pairs(Tpp._DEBUG_svars)do
svars[e]=a
end
TppSave.VarSave()end
end
function n.DEBUG_SetOrderBoxPlayerPosition()if next(Tpp._DEBUG_gvars)then
...
function n.DEBUG_SVarsClear()if next(Tpp._DEBUG_svars)then
for e,a in pairs(Tpp._DEBUG_svars)do
...
local n=s(n)e(n,{id="SetEnabled",enabled=false})e(n,{id="SetSneakRoute",route=o,point=a})e(n,{id="SetCautionRoute",route=o,point=a})if t then
e(n,{id="SetAlertRoute",enabled=true,route=o,point=a})else
e(n,{id="SetAlertRoute",enabled=false,route="",point=a})end
e(n,{id="SetEnabled",enabled=true})end
local a=s(r)e(a,{id="SetPosition",position=i,rotY=0})end
function n.DEBUG_WarpVehicleAndSoldier(o,r,t,i,a,l)if not s(o)then
o={o}end
local s=GameObject.GetGameObjectId
local e=GameObject.SendCommand
if not a then
...
local n=s(n)e(n,{id="SetEnabled",enabled=false})e(n,{id="SetSneakRoute",route=t,point=a})e(n,{id="SetCautionRoute",route=t,point=a})if l then
e(n,{id="SetAlertRoute",enabled=true,route=t,point=a})else
e(n,{id="SetAlertRoute",enabled=false,route="",point=a})end
e(n,{id="SetEnabled",enabled=true})end
local a=s(r)e(a,{id="SetPosition",position=i,rotY=0})end
n.DEBUG_SkipOnChangeSVarsLog={timeLimitforSneaking=true,timeLimitforNonAbort=true}function n.DEBUG_AddSkipLogSVarsName(e)n.DEBUG_SkipOnChangeSVarsLog[e]=true
end
function n.QARELEASE_DEBUG_Init()local e
do
return
end
...
e.AddDebugMenu("LuaSystem","ShowPlayerPartsType","bool",mvars.qaDebug,"showPlayerPartsType")end
function n.QAReleaseDebugUpdate()local e=svars
local s=mvars
local e=(nil).Print
local a=(nil).NewContext()if s.seq_doneDumpCanMissionStartRefrainIds then
...
local n=n.DEBUG_GetSysVarsLog()e(a,{.5,.5,1},"LuaSystem showSysVars")for s,n in ipairs(n)do
e(a,n)end
end
if gvars.DEBUG_showGameStatus then
e(a,"")e(a,{.5,.5,1},"LuaSystem gameStatus")for s,n in pairs(TppDefine.GAME_STATUS_TYPE_ALL)do
local n=TppGameStatus.IsSet("TppMain.lua",s)if n then
e(a," statusType = "..(tostring(s)..(", IsSet = "..tostring(n))))end
end
local n=TppGameStatus.IsSet("TppMain.lua","S_IS_BLACK_LOADING")if n then
e(a," statusType = "..(tostring"S_IS_BLACK_LOADING"..(", IsSet = "..tostring(n))))end
e(a,"UIStatus")local n={{CallMenu="INVALID"},{PauseMenu="INVALID"},{EquipHud="INVALID"},{EquipPanel="INVALID"},{CqcIcon="INVALID"},{ActionIcon="INVALID"},{AnnounceLog="SUSPEND_LOG"},{AnnounceLog="INVALID_LOG"},{BaseName="INVALID"},{Damage="INVALID"},{Notice="INVALID"},{HeadMarker="INVALID"},{WorldMarker="INVALID"},{HudText="INVALID"},{GmpInfo="INVALID"},{AtTime="INVALID"},{InfoTypingText="INVALID"}}for s,n in pairs(n)do
for n,s in pairs(n)do
if(TppUiStatusManager.CheckStatus(n,s)==true)then
e(a,string.format(" UI = %s, Status = %s",n,s))end
end
end
end
if s.qaDebug.showWeaponVars then
local n={"PRIMARY_HIP","PRIMARY_BACK","SECONDARY"}e(a,{.5,.5,1},"LuaSystem WeaponVars")for n,s in ipairs(n)do
local n=TppDefine.WEAPONSLOT[s]e(a,string.format("Slot:%16s : vars.initWeapons = %04d, vars.weapons = %04d",s,vars.initWeapons[n],vars.weapons[n]))end
for n=0,7 do
e(a,string.format("Slot:%d : vars.supportWeapons = %04d, vars.initSupportWeapons = %04d, gvars.ply_lastWeaponsUsingTemp = %04d",n,vars.supportWeapons[n],vars.initSupportWeapons[n],gvars.ply_lastWeaponsUsingTemp[n+TppDefine.WEAPONSLOT.SUPPORT_0]))end
for n=0,7 do
e(a,string.format("Slot:%d : vars.items = %04d, vars.initItems = %04d, gvars.ply_lastItemsUsingTemp = %04d",n,vars.items[n],vars.initItems[n],gvars.ply_lastItemsUsingTemp[n]))end
end
...
s(a,"")s(a,{.5,.5,1},"LuaSystem DBG.showSysSVars")for e,n in pairs(t.__as)do
if(n<=1)then
s(a,string.format(" %s = %s",tostring(e),tostring(t[e])))else
s(a,string.format(" %s = %s",tostring(e),tostring(n)))for n=0,(n-1)do
s(a,string.format("   %s[%d] = %s",tostring(e),n,tostring(t[e][n])))end
end
end
end
if o.showDebugPerfCheck then
s(a,{.5,.5,1},"LuaSystem DBG.showPerf")for o,e in pairs(l)do
s(a," perf["..(n.PERF_CHECK_TYPE[o]..("] = "..e)))end
end
if e.debug.AnimalBlock then
s(a,{.5,.5,1},"LuaSystem DBG.AnimalBlock")local o,t=Tpp.GetCurrentStageSmallBlockIndex()s(a,string.format("current block position (x,y) = (%03d, %03d)",o,t))s(a,"Load animal block area = "..tostring(e.animalBlockAreaName))local t=ScriptBlock.GetScriptBlockId"animal_block"local o
if t~=ScriptBlock.SCRIPT_BLOCK_ID_INVALID then
o=ScriptBlock.GetScriptBlockState(t)end
local t
...
s(a,"animal block state : "..tostring(t))if e.animalBlockScript then
s(a,"animalBlockScript exist")local o=""if e.animalBlockScript.OnMessage then
o="exist"else
o="  not"end
local t=""if e.animalBlockScript.OnReload then
t="exist"else
t="  not"end
s(a,"OnMessage "..(tostring(o)..(" OnReload "..tostring(t))))n.ShowMessageTable(a,"MessageTable",e.animalBlockScript.messageExecTable)else
if o==ScriptBlock.SCRIPT_BLOCK_STATE_INACTIVE or o==ScriptBlock.SCRIPT_BLOCK_STATE_ACTIVE then
s(a,{1,0,0},"this data is invalid!!!! please check data!!!")else
s(a,"animalBlockScript   not")end
end
end
if e.debug.ply_intelTrap then
s(a,{.5,.5,1},"LuaSystem PLY.intelTrap")for e,n in pairs(e.ply_intelTrapInfo)do
if Tpp.IsTypeString(e)then
s(a,{.5,1,.5},"intelName = "..tostring(e))for n,e in pairs(n)do
s(a,tostring(n)..(" = "..tostring(e)))end
end
end
end
if(o.showSubscriptMessageTable>0)then
s(a,{.5,.5,1},"LuaMessage subScripts")local s={"sequence","enemy","demo","radio","sound"}local s=s[e.debug.showSubscriptMessageTable]if s then
local o=TppMission.GetMissionName()..("_"..s)if e.rad_subScripts[s]then
local e=e.rad_subScripts[s]._messageExecTable
n.ShowMessageTable(a,o,e)end
end
end
if(o.showSequenceMessageTable>0)then
s(a,{.5,.5,1},"LuaMessage sequence")local s=TppSequence.GetSequenceNameWithIndex(e.debug.showSequenceMessageTable)if e.seq_sequenceTable then
local e=e.seq_sequenceTable[s]if e then
local e=e._messageExecTable
n.ShowMessageTable(a,s,e)end
end
end
if(o.showLocationMessageTable>0)then
s(a,{.5,.5,1},"LuaMessage location")end
if(o.showLibraryMessageTable>0)then
s(a,{.5,.5,1},"LuaMessage library")local e=Tpp._requireList[o.showLibraryMessageTable]local s=_G[e].messageExecTable
n.ShowMessageTable(a,e,s)end
if e.debug.showWeaponSelect then
s(a,{.5,.5,1},"LuaWeapon")if e.debug.weaponCategory<1 then
...
local t=e.debug.weaponCategoryList[e.debug.weaponCategory]s(a,{.5,1,.5},"Current weapon category : "..t[1])local l,d
local n,o,r=0,1,5
if e.debug.selectedWeaponId>0 then
o=e.debug.selectedWeaponId
end
for e,u in pairs(TppEquip)do
local g=string.sub(e,1,t[2])local i=false
...
l=u
d=e
s(a,{.5,1,.5},"> EquipId = TppEquip."..e)else
s(a,"  EquipId = TppEquip."..e)end
end
if n==(o+r)then
break
end
...
function n.ShowMessageTable(e,s,n)local a=(nil).Print
a(e,{.5,1,.5},s)if n==nil then
return
end
for s,n in pairs(n)do
local s=d(s)if n then
for o,n in pairs(n)do
local o=d(o)if n.func then
a(e,{1,1,1},s..(" : "..(o..(" : "..tostring(n.func)))))end
local n=n.sender
if n then
for n,t in pairs(n)do
a(e,{1,1,1},s..(" : "..(o..(" : Sender = "..(d(n)..(" : "..tostring(t)))))))end
end
end
end
end
end
function n.PerfCheckStart(a)local e=n
if((a<=0)and(a>#e.PERF_CHECK_TYPE))then
return
end
if(a==e.PERF_CHECK_TYPE.OnUpdate)then
if(i[e.PERF_CHECK_TYPE.OnUpdate]~=nil)then
t=t+(os.clock()-i[e.PERF_CHECK_TYPE.OnUpdate])end
end
i[a]=os.clock()end
function n.PerfCheckEnd(e,a)local o=mvars
local s=n
if((e<=0)and(e>#s.PERF_CHECK_TYPE))then
return
end
local d=a or""local n=0
local a=os.clock()-i[e]if(e==s.PERF_CHECK_TYPE.OnUpdate)then
if(t<p)then
if(a>r)then
r=a
end
...
l[e]=string.format("%4.2f",n*1e3)..("ms."..d)if o.debug and o.debug.showDebugPerfCheck then
if(n>1/60)then
else
...
function n.ErrorNotSupportYet(e)end
function n._GetColor(e)local e=n.Colors[e]if(e==nil)then
return nil
end
return e
end
