...
function e.OnFultonSoldier(o,i)local r=n(o,{id="GetStateFlag"})local _=n(o,{id="IsZombieOrMsf"})local n=n(o,{id="IsChild"})if _ then
if i then
...
elseif n then
e.AddTargetLifesavingHeroicPoint(n,i)else
...
e.SetAndAnnounceHeroicOgrePoint{heroicPoint="HEROIC_POINT_FULTONED_DYING_STAFF",ogrePoint="OGRE_POINT_FULTONED_DYING_STAFF"}end
end
else
...
function e.GetFobServerParameter(e)local n,o
if E(e)then
o=e
n=TppNetworkUtil.GetFobServerParameterByName(e)else
n=e
end
return n,o
end
function e.SetHeroicPoint(n)local e,n=e.GetFobServerParameter(n)if e<0 then
...
function e.SetOgrePoint(n)local e,n=e.GetFobServerParameter(n)if e<0 then
...
function e.AnnounceHeroicPoint(i,o,n)local o=o or"heroicPointDown"local n=n or"heroicPointUp"if vars.missionCode>=6e4 then
return
end
local e=e.GetFobServerParameter(i.heroicPoint)if e<0 then
TppUI.ShowAnnounceLog(o,-e)elseif e>0 then
TppUI.ShowAnnounceLog(n,e)end
end
...
local n=_(n)local n=e.BREAK_GIMMICK[n]if n then
...
function e.OnHelicopterLostControl(n,o)local i=_(n)local n=Tpp.IsLocalPlayer(o)if i==TppGameObject.GAME_OBJECT_TYPE_HELI2 then
if n then
...
elseif n then
PlayRecord.RegistPlayRecord"HERI_DESTROY"Tpp.IncrementPlayData"totalHelicopterDestoryCount"e.SetAndAnnounceHeroicOgrePoint(e.ENEMY_HELI_LOST_CONTROLE)TppUI.UpdateOnlineChallengeTask{detectType=27,diff=1}end
end
function e.SetAndAnnounceHeroicOgrePointForAnnihilateCp(i,o)local n
if o then
n="outpost_neutralize"else
n="guradpost_neutralize"end
e.SetAndAnnounceHeroicOgrePoint(i,nil,n)end
...
end},{msg="Dead",func=function(i,r,_,o)if r and Tpp.IsLocalPlayer(r)then
...
local r=TppEnemy.GetSoldierType(i)if(n(i,{id="IsDD"}))then
if(o~=nil)and(t(o,DeadMessageFlag.FIRE)~=0)then
...
local _=TppMission.IsFOBMission(vars.missionCode)and TppServerManager.FobIsSneak()local n=n(i,{id="GetStateFlag"})if(o~=nil)and(t(o,r)~=0)then
if not _ then
...
if o then
PlayRecord.RegistPlayRecord"BASE_SUPPRESSION"e.SetAndAnnounceHeroicOgrePointForAnnihilateCp(e.ON_ANNIHILATE_BASE,true)TppTrophy.Unlock(18)Tpp.IncrementPlayData"totalAnnihilateBaseCount"TppChallengeTask.RequestUpdate"ENEMY_BASE"TppUI.UpdateOnlineChallengeTask{detectType=32,diff=1}end
TppEmblem.AcquireOnCommandPostAnnihilated(n)elseif TppEnemy.IsOuterBaseCp(n)then
if o then
e.SetAndAnnounceHeroicOgrePointForAnnihilateCp(e.ON_ANNIHILATE_OUTER_BASE,false)Tpp.IncrementPlayData"totalAnnihilateOutPostCount"TppChallengeTask.RequestUpdate"ENEMY_BASE"TppTrophy.Unlock(18)TppUI.UpdateOnlineChallengeTask{detectType=33,diff=1}end
...
local n=mvars.ene_cpList[n]local i=TppCommandPost2.SetCpDominated{cpName=n,type=e}local n=TppCommandPost2.GetDominatedCpCount{type=e}local o=TppTrophy.DOMINATION_TARGET_CP_COUNT[e]if i then
end
if n==o then
...
function e.OnMessage(t,i,o,n,r,E,_)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,t,i,o,n,r,E,_)end
...
function e.UpdateHero()local e=gvars.isHero
local n=TppMotherBaseManagement.GetHeroicPoint()if(n>=vars.mbmHeroThreshold)then
gvars.isHero=true
end
