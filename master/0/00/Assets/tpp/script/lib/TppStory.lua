...
end,updateTiming={OnMissionStart=true,OnSideOpsClear=true}},{sub={"s11121"},condition=function()local t=TppQuest.IsNowOccurringElapsed()local n=e.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.STORY_SEQUENCE)if t or n then
...
end,detailCondition=function()TppDemo.UpdateHappyBirthDayFlag()local e=TppDemo.mtbsPriorityFuncList.HappyBirthDay()local n=TppMbFreeDemo.demoOptions.HappyBirthDay.GetNextDemoNameOrNil()~=nil
return e and n
...
end,radioList={"f6000_rtrg0360"}},QuietHasFriendshipWithChild={storyCondition=function()local n=e.GetCurrentStorySequence()==TppDefine.STORY_SEQUENCE.CLEARD_RETRIEVE_VOLGIN
local t=TppQuest.IsNowOccurringElapsed()local e=e.GetElapsedMissionCount(TppDefine.ELAPSED_MISSION_EVENT.STORY_SEQUENCE)==1
return n and(t or e)end,detailCondition=function()return TppDemo.mtbsPriorityFuncList.QuietHasFriendshipWithChild()end,demoName="QuietHasFriendshipWithChild"},QuietHasFriendshipWithChildRadio={storyCondition=function(e)return e.demoName=="QuietHasFriendshipWithChild"end,detailCondition=function()return true
...
function e.CheckAllMissionCleared()local o=true
local s=true
local a=true
local d=true
local r=true
...
local t=TppDefine.MISSING_NUMBER_MISSION_ENUM[e]if not t then
local t=tonumber(e)if(not gvars.str_missionClearedFlag[n])then
...
local n=true
local r={[10240]=true,[10115]=true,[10030]=true}if r[t]then
n=false
end
...
local i=TppDefine.MISSING_NUMBER_MISSION_ENUM[t]if not i then
local t=tonumber(t)n=n+TppUI.GetTaskCompletedNumber(t)e=e+TppUI.GetMaxMissionTask(t)end
end
return n,e
end
...
function e.CanOpenS10240()local t=e.GetClearedMissionCount{10010,10020,10030,10036,10043,10033,10040,10041,10044,10052,10054,10050,10070,10080,10086,10082,10090,10091,10195,10100,10110,10121,10115,10120,10085,10200,10211,10081,10130,10140,10150,10151,10045,10156,10093,10171}local n=e.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.STORY_SEQUENCE)local i=TppQuest.IsNowOccurringElapsed()local e=TppMotherBaseManagement.CanOpenS10240()if((t>=36)and e)and(n or i)then
return true
else
return false
end
end
function e.CanOpenS10260()local i=TppDemo.IsPlayedMBEventDemo"DecisionHuey"local r=TppBuddyService.CanSortieBuddyType(BuddyType.QUIET)local n=TppBuddyService.IsDeadBuddyType(BuddyType.QUIET)local t=TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.QUIET)local e=not TppUiCommand.IsUsingButterflyEmblem()if(((i and r)and(not n))and(t>=100))and e then
return true
else
return false
end
...
local i
repeat
r=e.GetStorySequenceTable(n)if r==nil then
return
end
local r=e.CheckNeedProceedStorySequence(r)if not r then
break
end
...
function e.IsOccuringBossQuiet()local e=e.GetClearedMissionCount{10041,10044,10052,10054}local t=TppMotherBaseManagement.IsBuiltMbMedicalClusterSpecialPlatform()local i=TppBuddy2BlockController.DidObtainBuddyType(BuddyType.QUIET)local n=TppBuddyService.CheckBuddyCommonFlag(BuddyCommonFlag.BOSS_QUIET_KILL)if((t and(e>=1))and(not i))and(not n)then
return true
else
return false
end
...
local s=t[1]local o=t[2]local t=e.radioDemoTable[o]local r=e._GetRadioList(t,i)if(not e.IsDoneEvent(t,s,n,o)and t.storyCondition(i))and t.detailCondition(i)then
if t.demoName then
...
e.DEBUG_InitQuestFlagsForTest()local n
repeat
local t=""for i,n in ipairs(TppDefine.MISSION_LIST)do
if e.IsMissionCleard(n)then
t=t..(","..tostring(n))end
end
coroutine.yield()TppTerminal.ReleaseMbSection()e.UpdateStorySequence{updateTiming="OnMissionClear",missionId=TppMission.GetMissionID()}e.DEBUG_SetNeedStoryTest(vars.missionCode)local t=e.GetForceMBDemoNameOrRadioList"forceMBDemo"e.GetForceMBDemoNameOrRadioList("blackTelephone",{demoName=t})e.GetForceMBDemoNameOrRadioList"freeHeliRadio"e.GetForceMBDemoNameOrRadioList"freeHeliRadio"repeat
coroutine.yield()TppQuest.UpdateActiveQuest{debugUpdate=true}until(not e.DEBUG_ClearQuestForTest(vars.missionCode))e.MissionOpen(10260)local t=e.DEBUG_GetUnclearedMissionCode()if mvars.str_DEBUG_needClearOneMission then
t=10036
...
vars.missionCode=t
local t=e.GetCurrentStorySequence()if t<TppDefine.STORY_SEQUENCE.STORY_FINISH then
n=false
else
n=true
end
coroutine.yield()until(n or mvars.str_testBreak)e.DEBUG_SkipDemoRadio=nil
end
...
local e=e.GetStorySequenceName(t)if TppDebugMbDevelop[e]then
