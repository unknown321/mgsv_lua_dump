...
local _="quest_cp"local d=EnemySubType or{}local function u(e)local t={}for n,i in ipairs(e)do
if s(i)then
t[n]=u(i)else
...
local i=0
while i<e do
local n=T(r,i)if n==a then
break
end
if e==1 then
e=t({type=r},{id="GetMaxInstanceCount"})if not e or e<1 then
break
end
end
local e=true
...
if e then
local e=t(n,{id="GetLifeStatus"})local t=t(n,{id="GetStatus"})if(t~=s)and(e~=TppGameObject.NPC_LIFE_STATE_DEAD)then
...
local a=t(i,{id="IsBroken"})local e=t(i,{id="IsFultonCaptured"})if(a==false)and(e==false)then
...
function e.SetSneakRoute(e,s,n,r)if not e then
return
end
if o(e)then
e=GameObject.GetGameObjectId(e)end
...
if e~=a then
t(e,{id="SetSneakRoute",route=s,point=n,isRelaxed=i})end
end
...
function e.AssignSoldiersToCP()local r=TppMission.GetMissionID()e._ConvertSoldierNameKeysToId(mvars.ene_soldierTypes)mvars.ene_soldierSubType=mvars.ene_soldierSubType or{}mvars.ene_soldierLrrp=mvars.ene_soldierLrrp or{}local n=e.subTypeOfCp
for a,p in pairs(mvars.ene_soldierIDList)do
local i=mvars.ene_cpList[a]local o=n[i]local s=false
for n,p in pairs(p)do
t(n,{id="SetCommandPost",cp=i})if mvars.ene_lrrpTravelPlan[a]then
...
function e._MakeShiftChangeUnit(t,i,n,r,o,d,l,a,T,u,_)if mvars.ene_noShiftChangeGroupSetting[t]and mvars.ene_noShiftChangeGroupSetting[t][n]then
return nil
end
...
local function d(n,e,a)local t=t(a,{id="GetPosition"})local e=e-t
...
for n,e in pairs(e)do
local n=p(e)local n=mvars.ene_usingGimmickRouteEnemyList[n]if n then
t(n,{id="SetSneakRoute",route=""})end
local n=mvars.gim_routeGimmickConnectTable[p(e)]if(n~=nil)and TppGimmick.IsBroken{gimmickId=n}then
...
function e.SetIgnoreDisableNpc(n,i)local e
if l(n)then
e=n
elseif o(n)then
e=r(n)else
...
t(e,{id="SetIgnoreDisableNpc",enable=i})return true
end
...
local i=e.IsEliminateTarget(n)local r=e.IsRescueTarget(n)if a then
