...
function e.UpdateObjective(i)if not mvars.mis_missionObjectiveDefine then
return
end
...
if o then
if not i then
mvars.mis_updateObjectiveRadioGroupName=TppRadio.GetRadioNameAndRadioIDs(n.radioGroups)end
local e=e.GetObjectiveRadioOption(n)TppRadio.Play(n.radioGroups,e)end
end
if s(t)then
if o then
local e=e.GetObjectiveRadioOption(t)if i then
...
function e.SetHelicopterDoorOpenTime(e)if not p(e)then
return
end
mvars.mis_helicopterDoorOpenTimerTimeSec=e
end
function e.UpdateCheckPoint(e)end
function e.UpdateCheckPointAtCurrentPosition()end
...
function e.AcceptEmergencyMission(n,i,t,s)if not e.IsEmergencyMission(n)then
return
end
...
e.AbortMission{emergencyMissionId=n,nextMissionId=o,nextLayoutCode=i,nextClusterId=t,nextMissionStartRoute=s,isNoSave=true,isInterrupt=true}end
function e.AcceptStartFobSneaking(i,n,s)e.SetNextMissionCodeForMissionClear(s)mvars.mis_nextLayoutCode=TppLocation.ModifyMbsLayoutCode(i)mvars.mis_nextClusterId=n
end
...
function e._StartEmergencyMissionTimer(n,i,s)if not r(n)then
return
end
if not p(i)then
return
...
function e.ExecuteRestartMission(n)e.SafeStopSettingOnMissionReload()TppQuest.OnMissionGameEnd()TppPlayer.ResetInitialPosition()TppMain.ReservePlayerLoadingPosition(TppDefine.MISSION_LOAD_TYPE.MISSION_RESTART)e.VarResetOnNewMission()local i
if n then
...
local n=function()e.RequestLoad(vars.missionCode,nil,{force=true})end
if i then
...
e.DisableInGameFlag()if vars.missionCode==10115 then
e.ResetEmegerncyMissionSetting()TppSave.VarRestoreOnMissionStart()e.RestartMission(n)else
e.ContinueFromCheckPoint(n)end
end
...
local n=function()local i
if e.IsFOBMission(n)then
i={waitOnLoadingTipsEnd=false}end
e.RequestLoad(vars.missionCode,n,i)end
if s then
e.ShowAnnounceLogOnFadeOut(n)else
n()end
end
...
if n then
TppMain.DisablePause()Player.SetPause()n()TppTerminal.AddStaffsFromTempBuffer()TppSave.VarSave()TppSave.SaveGameData(nil,nil,nil,true)end
return n
end
function e.AbortMission(n)local m
local u
local r
local a
local l
...
mvars.mis_missionAbortLoadingOption={}local i=e.IsHelicopterSpace(n)local a=e.IsFreeMission(n)local t=e.IsHelicopterSpace(mvars.mis_nextMissionCodeForAbort)local s=e.IsFreeMission(mvars.mis_nextMissionCodeForAbort)if mvars.mis_isInterruptMission then
...
TppPlayer.SaveCaptureAnimal()TppClock.SaveMissionStartClock()TppWeather.SaveMissionStartWeather()TppTerminal.AddStaffsFromTempBuffer()TppRevenge.OnMissionClearOrAbort(n)TppRevenge.SaveMissionStartMineArea()if gvars.solface_groupNumber>=4294967295 then
...
e.ClearMissionStartRecoverSoundDemo()TppPlayer.ResetMissionStartPosition()end
if t then
...
TppBuddyService.BuddyMissionInit()TppMain.ReservePlayerLoadingPosition(TppDefine.MISSION_LOAD_TYPE.MISSION_ABORT,i,a,t,s,mvars.mis_abortWithSave)TppWeather.OnEndMissionPrepareFunction()e.VarResetOnNewMission()gvars.mis_orderBoxName=0
if gvars.ini_isTitleMode then
mvars.mis_missionAbortLoadingOption.showLoadingTips=false
...
function e.LoadForMissionAbort()e.RequestLoad(vars.missionCode,mvars.mis_abortCurrentMissionCode,mvars.mis_missionAbortLoadingOption)end
...
function e.ReserveGameOver(e,n,i)if svars.mis_isDefiniteMissionClear then
return false
end
...
e.ResetNeedWaitMissionInitialize()end
function e.MissionFinalize(n)local l,r,i,t,a,o
if s(n)then
l=n.isNoFade
r=n.isExecGameOver
i=n.showLoadingTips
...
if i then
TppClock.SetTimeFromHelicopterSpace(mvars.mis_selectedDeployTime,l,vars.locationCode)end
...
e.ResetEmegerncyMissionSetting()TppSave.VarSaveMBAndGlobal()TppSave.VarRestoreOnContinueFromCheckPoint()TppMain.ReservePlayerLoadingPosition(TppDefine.MISSION_LOAD_TYPE.CONTINUE_FROM_CHECK_POINT)TppSave.SaveGameData(vars.missionCode)end
if TppRadio.playingBlackTelInfo then
mvars.mis_showLoadingTipsOnMissionFinalize=false
end
e.RequestLoad(vars.missionCode,n,{showLoadingTips=mvars.mis_showLoadingTipsOnMissionFinalize,waitOnLoadingTipsEnd=r,ignoreMtbsLoadLocationForce=mvars.mis_missionFinalizeIgnoreMtbsLoadLocationForce})end
...
function e.IsMbFreeMissions(n)local e={[30050]=true,[30150]=true,[30250]=true}if e[n]then
return true
else
return false
end
...
end,option={isExecMissionPrepare=true,isExecDemoPlaying=true}},{msg="Exit",sender="innerZone",func=function()mvars.mis_isAlertOutOfMissionArea=true
...
local n=o"FallDeath"local n=o"Suicide"function e.OnPlayerDead(e,e)end
...
e.MessageHandler={OnMessage=function(n,i,a,s,t,o)e.OnMessageWhileLoading(n,i,a,s,t,o)end}GameMessage.SetMessageHandler(e.MessageHandler,{"UI","Radio","Video","Network","Nt"})end
...
if s and i then
e.SetMissionObjectives(s,i,n)end
end
...
function e.OnMessageWhileLoading(o,t,a,r,s,i)local n=Tpp.DEBUG_StrCode32ToString
local n
Tpp.DoMessage(e.messageExecTableWhileLoading,e.CheckMessageOptionWhileLoading,o,t,a,r,s,i,n)end
function e.OnMessage(i,n,s,o,t,a,r)Tpp.DoMessage(e.messageExecTable,e.CheckMessageOption,i,n,s,o,t,a,r)end
...
function e.CheckMissionState(i,t,s,a)local n=mvars
local e=svars
if e==nil then
return
end
...
function e.CheckMissionClearOnOutOfMissionArea()e.MgoCheckAlertArea()return false
end
...
function e.MgoCheckAlertArea()local n=MpRulesetManager.GetActiveRuleset()local n=n.currentState
if not((n=="RULESET_STATE_ROUND_REGULAR_PLAY"or n=="RULESET_STATE_ROUND_OVERTIME")or n=="RULESET_STATE_ROUND_SUDDEN_DEATH")then
e.DisableAlertOutOfMissionArea()return false
end
...
function e.EnableAlertOutOfMissionArea()if e.MgoCheckAlertArea()then
end
end
...
local function u()if m"Timer_OutsideOfHotZoneCount"then
...
local T,f,p,d=e.GetSyncMissionStatus()local r=n.mis_isAlertOutOfMissionArea
local s=n.mis_isOutsideOfMissionArea
local u=n.mis_isOutsideOfHotZone
local c=o.mis_canMissionClear
if T and f then
TppMain.DisableGameStatus()HighSpeedCamera.RequestToCancel()e.EstablishedMissionClear(o.mis_missionClearType)elseif p and d then
...
elseif c then
e.UpdateAtCanMissionClear(u,s)else
if s then
...
local e=svars
local a=TppNetworkUtil.IsHost()local r=TppNetworkUtil.IsSessionConnect()local i=false
local n=false
local t=false
local s=false
if a then
...
local i=GameObject.SendCommand(e,{id="GetResourceId"})local n=not Tpp.IsHelicopter(vars.playerVehicleGameObjectId)TppTerminal.OnFulton(e,nil,nil,i,nil,n,PlayerInfo.GetLocalPlayerIndex())end
end
else
...
local n=e.GetMissionClearType()local t=e.EvaluateVehicleCarryOption(n)local s=nil
local i=nil
...
local t=Vector3(0,-.75,1.98)local e=Vector3(e[1],e[2],e[3])local t=-Quat.RotationY(TppMath.DegreeToRadian(n)):Rotate(t)s=t+e
i=n
end
end
end
end
Vehicle.SaveCarry{options=t,initialPosition=s,initialRotY=i}end
...
local s=_()local i=g()local n=not a(vars.playerVehicleGameObjectId)if o then
if i and n then
...
TppSoundDaemon.PostEvent"env_wormhole_out"e.ReserveGameOver(TppDefine.GAME_OVER_TYPE.OUTSIDE_OF_MISSION_AREA,TppDefine.GAME_OVER_RADIO.OUT_OF_MISSION_AREA)else
...
if TppLocation.IsAfghan()then
e.AbortMission{nextMissionId=TppDefine.SYS_MISSION_ID.AFGH_FREE,isNoSave=n,fadeDelayTime=a,fadeSpeed=i,presentationFunction=o,playRadio=t}elseif TppLocation.IsMiddleAfrica()then
e.AbortMission{nextMissionId=TppDefine.SYS_MISSION_ID.MAFR_FREE,isNoSave=n,fadeDelayTime=a,fadeSpeed=i,presentationFunction=o,playRadio=t}else
e.AbortMission{nextMissionId=TppDefine.SYS_MISSION_ID.AFGH_FREE,isNoSave=n,fadeDelayTime=a,fadeSpeed=i,presentationFunction=o,playRadio=t}end
end
...
local i=Vector3(0,-.75,1.98)local e=Vector3(e[1],e[2],e[3])local i=-Quat.RotationY(TppMath.DegreeToRadian(n)):Rotate(i)local e=i+e
local i=TppMath.Vector3toTable(e)local e=n
TppPlayer.SetInitialPosition(i,e)TppPlayer.SetMissionStartPosition(i,e)return true
end
end
...
e.ReserveMissionStartRecoverSoundDemo()else
e.ClearMissionStartRecoverSoundDemo()end
TppEnemy.FultonRecoverOnMissionGameEnd()TppPlayer.SaveCaptureAnimal()TppTerminal.AddVolunteerStaffs()if e.systemCallbacks.OnMissionGameEndFadeOutFinish then
...
function e.SetMissionObjectives(n,e,i)mvars.mis_missionObjectiveDefine=n
...
local i=mvars.mis_missionObjectiveDefine[n]if i then
e.SetMissionObjectiveEnable(n,false)e.DisableObjective(i)end
end
end
...
TppOutOfMissionRangeEffect.Disable(1)TppTerminal.PlayTerminalVoice("VOICE_WARN_MISSION_AREA",false)end
function e.VarResetOnNewMission()TppScriptVars.InitForNewMission()TppPlayer.UnsetRetryFlag()if GameConfig.GetStealthAssistEnabled()then
mvars.mis_needSaveConfigOnNewMission=true
GameConfig.SetStealthAssistEnabled(false)end
TppPlayer.ResetStealthAssistCount()TppSave.ReserveVarRestoreForMissionStart()e.SetNextMissionCodeForMissionClear(c)e.ResetMissionClearState()end
...
function e.GoToEmergencyMission()local s=gvars.mis_nextMissionCodeForEmergency
local t
...
e.ReserveMissionClear{missionClearType=TppDefine.MISSION_CLEAR_TYPE.FROM_HELISPACE,nextMissionId=s,nextHeliRoute=t,nextLayoutCode=n,nextClusterId=i}end
function e.RequestLoad(e,n,i)if not mvars then
return
end
TppMain.EnablePause()mvars.mis_loadRequest={nextMission=e,currentMission=n,options=i}end
function e.LoadWithChunkCheck()local t,n,i=mvars.mis_loadRequest.nextMission,mvars.mis_loadRequest.currentMission,mvars.mis_loadRequest.options
local s=Tpp.GetChunkIndex(vars.locationCode)if e.IsChunkLoading(s)then
return
end
e.Load(t,n,i)mvars.mis_loadRequest=nil
end
...
TppMain.EnablePause()TppMain.EnableBlackLoading(s)if(i~=n)or(e and e.force)then
local s=TppPackList.GetLocationNameFormMissionCode(n)local s=TppPackList.GetLocationNameFormMissionCode(i)local s
if TppSystemUtility.GetCurrentGameMode()=="TPP"then
...
function e.SetPlayRecordClearInfo()end
...
function e._CreateMissionName(e)return tostring(e)end
function e._PushReward(e,n,i)TppReward.Push{category=e,langId=n,rewardType=i}end
return e