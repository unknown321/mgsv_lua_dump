...
function e.EnableGameStatus(t,e)local n=e or{}local e=TppUI.GetOverrideGameStatus()if e then
for e,a in pairs(e)do
n[e]=a
end
end
Tpp.SetGameStatus{target=t,except=n,enable=true,scriptName="TppDemo.lua"}end
...
function e.FinalizeOnDemoBlock()if l()then
...
local e,a,n=DemoDaemon.GetStartPosition(e,"Player")if not e then
return
end
local e=Tpp.GetRotationY(n)local e={position=a,direction=e}return e
end
function e.PlayOpening(u,n)local s=n or{}s.isSnakeOnly=false
local l="_openingDemo"local a="p31_020000"local n={"p31_020000","p31_020001","p31_020002"}local t=math.random(#n)a=n[t]e.AddDemo(l,a)local r,t
local n,o
local m=Vector3(vars.playerPosX,vars.playerPosY,vars.playerPosZ)local i=Vector3(0,0,1.98)local d=Quat.RotationY(TppMath.DegreeToRadian(vars.playerRotY))if gvars.mis_orderBoxName~=0 and mvars.mis_orderBoxList~=nil then
local e=TppMission.FindOrderBoxName(gvars.mis_orderBoxName)if e~=nil then
n,o=TppMission.GetOrderBoxLocatorByTransform(e)end
end
if n then
...
DemoDaemon.SetDemoTransform(a,t,r)e.Play(l,u,s)end
function e.PlayGetIntelDemo(d,l,i,n,o)local t=n or{}t.isSnakeOnly=false
local n,a
if o then
n,a="p31_010026","p31_010026_001"else
n,a="p31_010025","p31_010025_001"end
local r="_getInteldemo"local o="_getInteldemo02"e.AddDemo(r,n)e.AddDemo(o,a)local a,o=Tpp.GetLocatorByTransform(l,i)local i=Tpp.GetRotationY(o)Player.RequestToSetTargetStance(PlayerStance.STAND)if a~=nil then
DemoDaemon.SetDemoTransform(n,o,a)e.Play(r,d,t)TppUI.ShowAnnounceLog"getIntel"end
end
function e.IsNotPlayable()return false
end
...
Player.SetPause()end
function e.SetPlayerWarp(e,n)mvars.dem_isPlayerPausing=true
Player.SetPause()local a={type="TppPlayer2",index=0}local e={id="WarpAndWaitBlock",pos={e:GetX(),e:GetY(),e:GetZ()},rotY=n}GameObject.SendCommand(a,e)end
...
function e.CheckEventDemoDoor(a,n,e)local o=TppPlayer.GetPosition()local t=30
if a==nil then
return false
end
...
local e=0
local l,i=0,1
local r=Tpp.IsNotAlert()local t=TppEnemy.IsActiveSoldierInRange(o,t)local n
...
Player.SetEventLockDoorIcon{doorId=a,isNgIcon=e}return n,r,(not t)end
...
function e.OnMessage(t,o,r,i,l,a,n)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,t,o,r,i,l,a,n)end
...
function e.DoOnEndMessage(n,o,a,r,t)if(not o)then
local e=true
if r and(not t)then
e=false
end
if e then
TppUI.FadeIn(TppUI.FADE_SPEED.FADE_NORMALSPEED,"DemoSkipFadeIn",mvars.dem_currentSkippedScdDemoID,{exceptGameStatus=a})end
end
...
local e={missionStateCheck=false,gameCameraInterpedToDemo=o,demoBlockLoaded=t,playerModelReloaded=a,playerActionAllowed=n,playerMoveToPosition=r,waitTextureLoadOnDemoPlay=i}return e
end
...
function e.CanStartPlay(r,t)local n=true
for a,o in pairs(t)do
if o==false then
local e=e.PLAY_REQUEST_START_CHECK_FUNC[a](r)if e then
t[a]=true
else
n=false
end
...
function e.CanFinishPlay(r,t)local n=true
for a,o in pairs(t)do
if o==false then
local e=e.FINISH_WAIT_CHECK_FUNC[a](r)if e then
t[a]=true
else
n=false
end
...
function e._OnDemoInit(e)end
function e._OnDemoPlay(e)end
function e._OnDemoEnd(e)end
function e._OnDemoInterrupt(e)end
function e._OnDemoSkip(e)end
function e._OnDemoDisable(e)end
...
local e=TppServerManager.GetNuclearAbolitionCount()local n=e>=0
...
local n=TppBuddyService.DidObtainBuddyType(BuddyType.DOG)local a=not TppBuddyService.CanSortieBuddyType(BuddyType.DOG)local t=TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.D_DOG_COME_TO_GET)return((e and n)and a)and t
end,DdogGoWithMe=function()local a=TppStory.GetClearedMissionCount{10041,10044,10052,10054}>=3
local e=not TppBuddyService.CanSortieBuddyType(BuddyType.DOG)local n=TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.D_DOG_GO_WITH_ME)return(a and e)and n
...
end,LongTimeNoSee_DdogLowLikability=function()local n=TppStory.IsMissionCleard(10050)local e=gvars.elapsedTimeSinceLastPlay>=t
...
end,LongTimeNoSee_DdogHighLikability=function()local a=TppStory.IsMissionCleard(10050)local t=gvars.elapsedTimeSinceLastPlay>=t
local e=TppBuddyService.CanSortieBuddyType(BuddyType.DOG)local n=25<=TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.DOG)and TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.DOG)<75
return((a and t)and e)and n
end,LongTimeNoSee_DdogSuperHighLikability=function()local a=TppStory.IsMissionCleard(10050)local n=gvars.elapsedTimeSinceLastPlay>=t
local e=TppBuddyService.CanSortieBuddyType(BuddyType.DOG)local t=TppBuddyService.GetFriendlyPoint(BuddyFriendlyType.DOG)>=75
...
local a=TppStory.IsMissionCleard(10040)local e=0
...
local e=TppUiCommand.IsBirthDay()local n=TppStory.GetCurrentStorySequence()>=TppDefine.STORY_SEQUENCE.CLEARD_TO_MATHER_BASE
...
local a=(vars.buddyType==BuddyType.QUIET)local n=TppStory.CanArrivalQuietInMB(false)return((e and t)and a)and n
end
...
local e=TppStory.CanArrivalQuietInMB(true)local t=TppStory.GetCurrentStorySequence()>=TppDefine.STORY_SEQUENCE.CLEARD_OKB_ZERO
local a=not(TppQuest.IsOpen"outland_q20913"or TppQuest.IsOpen"lab_q20914")return((n and e)and t)and a
end
...
local n=TppStory.IsMissionCleard(10086)local t=TppStory.CanArrivalQuietInMB(false)local a=Player.GetSmallFlyLevel()>=1
...
local a=TppStory.CanArrivalQuietInMB(true)local e=TppQuest.IsOpen"sovietBase_q99030"local n=not TppRadio.IsPlayed"f2000_rtrg8290"return(a and e)and n
end
...
local e=TppStory.GetCurrentStorySequence()>=TppDefine.STORY_SEQUENCE.CLEARD_OKB_ZERO
local n=TppStory.IsMissionCleard(10130)return e and n
...
function e.IsUseMBDemoStage(e)if e then
...
function e.SetNextMBDemo(e)local n=TppDefine.MB_FREEPLAY_DEMO_ENUM[e]if e and n then
...
function e.IsBattleHangerDemo(n)local e={"DevelopedBattleGear1","DevelopedBattleGear2","DevelopedBattleGear4","DevelopedBattleGear5"}for a,e in ipairs(e)do
if e==n then
return true
end
end
...
function e.EnableNpc(t)local n=mvars.dem_demoFlags[t]or{}if not n.isInGame then
local a="all"local t=mvars.dem_demoList[t]if n.finishFadeOut or mvars.dem_isSkipped[t]then
a={}for n,e in pairs(TppDefine.GAME_STATUS_TYPE_ALL)do
a[n]=e
end
end
e.EnableGameStatus(a,n.exceptGameStatus)end
