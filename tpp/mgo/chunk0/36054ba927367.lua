local e={}local S=TppScriptVars.IsSavingOrLoading
e.saveQueueDepth=0
...
function e.IsSaving(n)if TppScriptVars.IsSavingOrLoading()then
return true
end
...
function e.HasQueue(n)for a=1,e.saveQueueDepth do
...
function e.SaveGameData(e,e,e,e)end
function e.GetSaveGameDataQueue(e,e,e)end
function e.SaveConfigData(n,a,t)if a then
local n=e.MakeNewSaveQueue(TppDefine.SAVE_SLOT.CONFIG,TppDefine.SAVE_SLOT.CONFIG_SAVE,TppScriptVars.CATEGORY_CONFIG,TppDefine.CONFIG_SAVE_FILE_NAME,n)return e.DoSave(n,true)elseif t then
...
function e.SavePersonalData(n,a,t)if a then
local n=e.MakeNewSaveQueue(TppDefine.SAVE_SLOT.PERSONAL,TppDefine.SAVE_SLOT.PERSONAL_SAVE,TppScriptVars.CATEGORY_PERSONAL,TppDefine.PERSONAL_DATA_SAVE_FILE_NAME,n)return e.DoSave(n,true)elseif t then
...
e.DO_RESERVE_SAVE_FUNCTION={[TppDefine.CONFIG_SAVE_FILE_NAME]=e.SaveConfigData,[TppDefine.PERSONAL_DATA_SAVE_FILE_NAME]=e.SavePersonalData}function e.ReserveNextMissionStartSave(n)if not e.DO_RESERVE_SAVE_FUNCTION[n]then
return
end
e.missionStartSaveFilePool=e.missionStartSaveFilePool or{}e.missionStartSaveFilePool[n]=true
end
...
local e=e.DO_RESERVE_SAVE_FUNCTION[n]e()end
e.missionStartSaveFilePool=nil
end
...
function e._SaveMissionData(a)local t,t,n,t=TppMission.GetSyncMissionStatus()if n then
end
return e.AddSlotToSaveQueue(a,TppDefine.SAVE_SLOT.CHECK_POINT,TppDefine.SAVE_SLOT.SAVING,TppScriptVars.CATEGORY_MISSION)end
...
function e.MakeNewGameSaveData(n)TppVarInit.InitializeOnNewGameAtFirstTime()TppVarInit.InitializeOnNewGame()if n then
TppTerminal.AcquirePrivilegeInTitleScreen()end
e.VarSave(vars.missionCode,true)e.VarSaveOnRetry()local a=e.GetSaveGameDataQueue(vars.missionCode)local a=e.DoSave(a,true)if n then
e.CheckAndSavePersonalData()end
return a
end
function e.GetIntializedCompositSlotSaveQueue(a,n,e)return{fileName=a,needIcon=n,doSaveFunc=e}end
...
function e.MakeNewSaveQueue(t,a,i,r,n,S)local e={}e.slot=t
e.savingSlot=a
e.category=i
e.fileName=r
e.needIcon=n
...
a=n.category[S]p=e.GetSaveFileVersion(a)TppScriptVars.CopySlot({n.savingSlot,t},t)end
else
a=n.category
if a then
...
if t then
t()end
local e=TppScriptVars.WriteSlotToFile(n.savingSlot,S,i)if r then
gvars.sav_SaveResultCheckFileName=Fox.StrCode32(S)end
return e
end
function e.Update()if(not S())then
if(gvars.sav_SaveResultCheckFileName~=0)then
local n=true
...
function e.OnMessage(n,a,t,S,i,p,r)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,n,a,t,S,i,p,r)end
function e.WaitingAllEnqueuedSaveOnStartMission()while S()do
e.CoroutineYieldWithShowSavingIcon()end
while T()do
e.ProcessSaveQueue()while S()do
e.CoroutineYieldWithShowSavingIcon()end
end
end
function e.CoroutineYieldWithShowSavingIcon()TppUI.ShowSavingIcon()coroutine.yield()end
function e.SaveVarsToSlot(a,t,n)local e=e.GetSaveFileVersion(n)TppScriptVars.SaveVarsToSlot(a,t,n,e)end
...
function e.VarSaveMbMangement(n)if e.CanSaveMbMangementData(n)then
...
function e.LoadFromSaveFile(n,e)return TppScriptVars.ReadSlotFromFile(n,e)end
...
function e.LoadGameDataFromSaveFile()end
local t={TppScriptVars.CATEGORY_GAME_GLOBAL,TppScriptVars.CATEGORY_MISSION,TppScriptVars.CATEGORY_RETRY,TppScriptVars.CATEGORY_MB_MANAGEMENT,TppScriptVars.CATEGORY_QUEST,TppDefine.CATEGORY_MISSION_RESTARTABLE}function e.CheckGameDataVersion()for a,n in ipairs(t)do
...
function e.CopyGameDataFromSavingSlot()for e,n in ipairs(t)do
...
function e.CheckSlotVersion(n,a,S)if n==TppDefine.CATEGORY_MISSION_RESTARTABLE and TppDefine.CATEGORY_MISSION_RESTARTABLE==TppScriptVars.CATEGORY_MGO then
return TppDefine.SAVE_FILE_LOAD_RESULT.OK
end
local t=e.GetSaveFileVersion(n)local e=TppDefine.SAVE_FILE_INFO[n].slot
if S then
