...
function e.OpenChildSoldier_1()local t=e.IsCleard"mtbs_q99050"local n=e.IsNowOccurringElapsed()local a=TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.STORY_SEQUENCE)local e=TppStory.IsMissionCleard(10093)return((a or n)and t)and e
end
...
function t.mtbs_q99011()local n=TppBuddy2BlockController.DidObtainBuddyType(BuddyType.QUIET)local e=TppStory.IsMissionCleard(10086)local t=TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.QUIET_VISIT_MISSION)return(n and e)and t
end
...
local o=t.questName
local s=e.IsActiveOnMBTerminal(t)local a=e.IsCleard(o)if t and(s or a)then
t.index=i
t.isActive=s
t.isCleard=a
t.gmp=e.GetBounusGMP(o)table.insert(n,t)end
end
end
...
function r.Mtbs_child_dog()local e=TppBuddyService.CanSortieBuddyType(BuddyType.DOG)local t=not TppStory.IsNowOccurringElapsedMission(TppDefine.ELAPSED_MISSION_EVENT.D_DOG_GO_WITH_ME)return(not e)and t
end
function r.Mtbs_ddog_walking()local e=TppBuddyService.IsDeadBuddyType(BuddyType.DOG)return(not e)end
function r.cliffTown_q99080()local t=TppMotherBaseManagement.IsExistStaff{uniqueTypeId=110}local e=TppStory.IsMissionCleard(10240)return t or e
end
...
local i,n=TppEnemy.GetQuestCount()local a,o=TppGimmick.GetQuestShootingPracticeCount()if i>0 and n>1 then
e.ShowAnnounceLog(s.UPDATE,t,i,n)elseif a>0 and o>1 then
e.UpdateShootingPracticeUi()e.ShowAnnounceLog(s.UPDATE,t,a,o)end
end
...
function e.Messages()local t=e.DeactiveQuestAreaTrapMessages()return h{Block={{msg="StageBlockCurrentSmallBlockIndexUpdated",func=e.OnUpdateSmallBlockIndex}},UI={{msg="EndFadeOut",sender="FadeOutOnOutOfMissionArea",func=function()mvars.qst_blockStateRequest=i.DEACTIVATE
TppUI.FadeIn(TppUI.FADE_SPEED.FADE_NORMALSPEED)end},{msg="QuestAreaAnnounceText",func=function(t)e.OnQuestAreaAnnounceText(t)end}},Marker={{msg="ChangeToEnable",func=function(t,a,n,s)e._ChangeToEnable(t,a,n,s)end}},Timer={{msg="Finish",sender="TimerShootingPracticeStart",func=function()e.StartShootingPractice()end,option={isExecMissionPrepare=true,isExecMissionClear=true}},{msg="Finish",sender="TimerShootingPracticeEnd",func=function()e.OnQuestShootingTimerEnd()end,option={isExecMissionPrepare=true,isExecMissionClear=true}},{msg="Finish",sender="TimerShootingPracticeRetryConfirm",func=function()TppGimmick.SetQuestShootingPracticeTargetInvisible()end,option={isExecMissionPrepare=true,isExecMissionClear=true}}},Trap=t}end
function e.OnMessage(i,r,t,o,s,n,a)Tpp.DoMessage(e.messageExecTable,TppMission.CheckMessageOption,i,r,t,o,s,n,a)local l=mvars.qst_questScriptBlockMessageExecTable
if l then
local e=a
local a
Tpp.DoMessage(l,TppMission.CheckMessageOption,i,r,t,o,s,n,e)end
...
local a=a
local l
Tpp.DoMessage(e,TppMission.CheckMessageOption,i,r,t,o,s,n,a)end
end
end
end
...
function e.RegisterQuestPackList(t,i)if not o(t)then
return
end
i=i or S
...
local i=e.GetRandomFaceId(t)local o={i}local s=TppSoldierFace.GetFaceFpkFileCodeList{face=o}if s==nil then
if n.gender==TppDefine.QUEST_GENDER_TYPE.MAN then
e.SetRandomFaceId(t,TppDefine.QUEST_FACE_ID_LIST.DEFAULT_MAN)elseif n.gender==TppDefine.QUEST_GENDER_TYPE.WOMAN then
e.SetRandomFaceId(t,TppDefine.QUEST_FACE_ID_LIST.DEFAULT_WOMAN)end
i=e.GetRandomFaceId(t)o={i}s=TppSoldierFace.GetFaceFpkFileCodeList{face=o}end
if s~=nil then
...
TppScriptBlock.RegisterCommonBlockPackList(i,a)end
function e.SetDefaultQuestBlock()mvars.qst_blockName=S
end
...
local a,s=Tpp.GetCurrentStageSmallBlockIndex()if e.IsInsideArea("activeArea",t,a,s,n)then
...
if s>0 and e>0 then
local s=n.qst_questList[s]if s then
local o=s.infoList[e]if o then
...
function e.OnUpdateSmallBlockIndex(a,n,s)local t=e.GetQuestBlockState()if t==nil then
return
end
...
e.UpdateQuestBlockStateAtNotLoaded(a,n,s)elseif(t==i)then
e.UpdateQuestBlockStateAtInactive(a,n,s)elseif(t==o)then
e.UpdateQuestBlockStateAtActive(a,n)end
end
...
function e.UpdateQuestBlockStateAtInactive(a,t)local n=e.GetCurrentQuestTable()if not e.IsInsideArea("loadArea",n,a,t)then
...
function e.ResetQuestStatus()gvars.qst_currentQuestName=m
gvars.qst_currentQuestStepNumber=c
end
...
function e.GetQuestName(n)for t,e in ipairs(u)do
local t=tonumber(string.sub(e.questId,-5))if n==t then
return e.questName
end
end
end
...
local n=nil
local n={}local n={}local i={}local o={}for t,a in ipairs(t.infoList)do
local t=a.name
local s=TppDefine.QUEST_INDEX[t]if s then
gvars.qst_questActiveFlag[s]=false
local s=r[t]if e.IsOpen(t)and(not s or s())then
...
TppUiCommand.SetSideOpsListUpdate()for t,e in ipairs(f)do
...
local a={waterway_q99012="waterway",tent_q99040="tent",tent_q20910="tent",sovietBase_q20912="waterway",fort_q20911="fort"}function e.OpenAndActivateSpecialQuest(t)local n=true
for a,t in ipairs(t)do
if e.CanOpenAndActivateSpecialQuest(t)then
e.OpenQuestForce(t)e.SwitchActiveQuest(t)e.AddStaffsFromTempBuffer()e.Save()else
n=false
end
end
if n then
...
function e.SwitchActiveQuest(n)local e=a[n]if not e then
return
end
for a,t in ipairs(TppQuestList.questList)do
...
local t=e.name
local e=TppDefine.QUEST_INDEX[t]if e then
gvars.qst_questActiveFlag[e]=(t==n)end
end
end
end
end
...
if e==n then
TppTerminal.AcquireKeyItem{dataBaseId=t,isShowAnnounceLog=true}for n,t in pairs(g)do
if e==n then
...
function e.GetClearEmblem(e)local e=I[e]if e then
...
function e.ShowAnnounceLog(n,t,r,i)if not n then
return
end
if n==s.OPEN then
...
local a=e.GetQuestNameLangId(t)if a~=false then
local n=_[t]local o,i=TppEnemy.GetQuestCount()local s,r=TppGimmick.GetQuestShootingPracticeCount()if n then
if o>1 then
TppUI.ShowAnnounceLog(n,o,i)elseif s>1 then
TppUI.ShowAnnounceLog(n,s,r)end
end
TppUI.ShowAnnounceLog"quest_list_update"TppUI.ShowAnnounceLog("quest_complete",a)local e=e.GetQuestNameId(t)if(e~=false)and(e~="q99012")then
...
TppUI.ShowAnnounceLog(e,r,i)end
end
end
...
local a={ruins_q60115=TppMotherBaseManagementConst.DESIGN_2027,sovietBase_q60110=TppMotherBaseManagementConst.DESIGN_2020,citadel_q60112=TppMotherBaseManagementConst.DESIGN_2021,outland_q60113=TppMotherBaseManagementConst.DESIGN_2022,pfCamp_q60114=TppMotherBaseManagementConst.DESIGN_2023,sovietBase_q60111=TppMotherBaseManagementConst.DESIGN_2024}local t={pfCamp_q39012={dataBaseId=TppMotherBaseManagementConst.ANIMAL_130},waterway_q39010={dataBaseId=TppMotherBaseManagementConst.ANIMAL_610},lab_q39011={dataBaseId=TppMotherBaseManagementConst.ANIMAL_2250}}function e.AcquireKeyItemOnMissionStart()for t,n in pairs(q)do
